...
import (
	...
	"github.com/caddyserver/caddy/v2/modules/caddy-l4/modules/l4proxyprotocol"
	"github.com/caddyserver/caddy/v2/modules/caddy-l4/modules/l4tls"
...
func (u *uConn) Read(b []byte) (n int, err error) {
	...
	for _, up := range upConns {
		...
		go func(up net.Conn) {
			...
			if uc, ok := up.(*net.UDPConn); ok {
				tConn := &uConn{Conn: uc}
				if _, err := io.Copy(down, tConn); err != nil {
