...
import (
	...
	"errors"
	"fmt"
	"github.com/caddyserver/caddy/v2/modules/caddyhttp/tproxy"
	"io"
	"net"
	"net/http"
	"net/url"
	...
	"github.com/caddyserver/caddy/v2/modules/caddyhttp/forwardproxy/httpclient"
	"go.uber.org/zap"
...
func (h *Handler) Provision(ctx caddy.Context) error {
	...
	if h.IpTransparent {
		var err error
		h.tProxy, err = tproxy.NewMmProxy(h.IpNet, h.Mark, h.Table)
		if err != nil {
			return err
		}
		h.tProxy.AddRule()
		h.tProxy.AddIptables()
	}
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	if r.Method == http.MethodConnect {
		...
		if h.IpTransparent {
			sIp := net.ParseIP(r.Header.Get("SourceIp"))
			targetConn, err = h.tProxy.TcpIpTRANSPARENTConn(sIp, hostPort)
			if err != nil {
				h.logger.Error("TcpIpTRANSPARENTConn", zap.String("source ip", sIp.String()),
					zap.Error(err))
			}
		}
